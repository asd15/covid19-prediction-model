...
st.cache()
def recovered_display():
    st.write("""
        # Covid19 Prediction App
        This page predicts the **Recovered cases** suffering from the virus in India!
        """)
    ...
    future_predict = sarimax_mod.predict(start=datetime.date.today(),
                                         end=datetime.date.today() + datetime.timedelta(days=date_sel), dynamic=True, )
    ...
    plt.tight_layout()
    st.write(f'**Forecast of upcoming {date_sel} days Covid-19 Recoveries in {sel_state}**')
    plt.show()
    st.pyplot(figg)
